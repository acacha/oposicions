...
void mostrarsolucio(POSICIO recorregut[],int files, int columnes);

void visualitza(int files,int columnes,int *p);

int main(int argc, char **argv) {
	//Declaració de variables
	int i,j,k, num_posicio;
	POSICIO pos_inicial;
	POSICIO recorregut[];
	int files,columnes;
	int tauler[][];
	...
	if (argc!=5)
		printf("Utilitza %d x_inicial y_inicial files columnes", argv[0]);
	
	//Inicialitza recorregut:
	files=atoi(argv[3]);
	columnes=atoi(argv[4]);
	recorregut = (POSICIO*) malloc(sizeof(posicio)*files*columnes);
	
	for (k = 0; k < files*columnes; ++k) {
			recorregut[k].x=-1;
			recorregut[k].y=-1;	
	}
		
	tauler = (int*) malloc(sizeof(int)*files*columnes);
	
	for (i = 0; i < MAX_FILES; ++i) {
		for (j = 0; j < MAX_COLUMNES; ++j) {
			tauler[i][j]=-1;
		}
	}
